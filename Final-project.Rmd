...
author: "Tommy Ruggles, Zev Rosen, Stefan Broekhuizen, Matt Sachs"
date: "12/12/2019"
...
```
The goal of our analysis is to build a series of a graphs to analyze women's tennis match data. Our data consists of three CSV files. Our first datafile contains a list of players, the second file has historical ranking data, and the third datafile consists of match record data. Our analysis is based on the project which can be viewed at the following link: https://towardsdatascience.com/extensive-analysis-of-women-tennis-matches-5c98333f338a. Our goal is to create a series of plots which shed light on historical trends in women's tennis match data. 
```{r load in libraries}
library(tidyverse)
library(stringr)
library(lubridate)
...
This graph shows a barplot of the players with the most grand slam titles. Serena and Venus Williams have the most over this time period. We filter the match data to only include grand slam finals. We then summarize the subset to count the number of grand slams won by each player and create our barplot. The x-axis contains the name of each player, and the height of the bar indicates the number of grand slams won. 
...
This is a graph showing Venus Williams' rankings from her time coming on tour until 2017. We see Williams get to the top 10 in the rankings in the late 1990's. Aside from a downturn in 2012 and 2013, she has stayed near the top of the rankings since. To clean the data, we filter the rankings data to only include rankings for Venus Williams, and make a new YearMonth variable. The x-axis of the plot shows the date and y-axis shows Venus' ranking at the specified date. 
...
These barplots depict the numbers of players who have represented their country from 2000-2017. The original graph, recreated on the right, displays the total number of players by country. The graph on the left displays the total number of players by country in the Australian Open. We compare the two to see whether demographics change depending on the tournament: for example, if Australian players are more represented in the Australian Open than in general.To create the Australian Open data, we filter matches data by tournament and selected winners and losers from each match. For both datasets, we use a unique rbind to create a list of players in the Australian Open and all tournaments. The x-axis divides players by country, and the height of the plot shows the number of players from each country. 
...
This barplot compares the age distributions of Wimbledon winners to losers. The graph indicates that there is greater variety in winners’ ages than in losers’ ages. Losers also tend to be younger, potentially indicating that more experienced players fare better. We filter the match data to only include Wimbledon finals, then mutate the data to create a variable represeneting age differenece. We use the histogram function to create our plots. The x-axis displays the age of each player, and y-axis displays the amount of winners or losers of a specified age.
...
Here, we are finding how many upsets occured in grand slams and see that Wimbledon, due to unique seeding and surface, has the most upsets as a whole. We first mutate the data to create variables for age difference and ranking difference, then create datasets for each grandslam and merge them together. We filter the binding dataset to create matches where upsets could occur, judging an upset by a ranking difference of at least 10. We also create a dataset to only include upsetts in grand slam finals. 
```{r upsetdata}
...
We classify a game as an "upset" if the winner rank is greater than or equal to the loser rank by 10 slots. We plot the ranking difference between winners and losers for the grand slams in Finals and Semi-Finals. We create barplots for grand slam finals and semifinals where the x-axis represents the tournament and height of the bar shows the number of upsets. To create the plot of grand slam upsets in semifinals, we filter the grand slam dataset to only include semifinals. 
...
We are now calling upsets in Grand Slam tournaments of 50+ spots "huge upsets," and tracking them through every round. We filter the grandslam data set to only include matches with a rankning difference of at least 50 and mutate the data to divide the data by round in the torunament. We then create a stacked bar plot where the x-axis represents the tournament name, the y-axis represents the number of upsets by each tournament and the fill shows the round in which the upset occurred. 
...
This a chloropleth which displays female grand slam players by country over the last 17 years. A darker shade means more players from each country have been represented. We filter the match dataset to only include grand slams and then select name and country for each winner and loser. We use an unique rbind to compile a list of players and summarize the data to count the number of players by country. To create our chloropleth, we first load in world map data and rename the countries to abbreviations using the countrycode library. On our chloropleth, the x-axis shows longitude, the y-axis latitude, and the darkness of the fill displays the number of players from each country.
...
This graph shows female grand slam winner age over time for the last 17 years. Each grand slam is represented by a different color line. Age tends to trend upwards over time for each grand slam though there are a high amount of fluctuations. We filter the dataset to only include grand slam finals and then mutate the data to make the names "French Open" and "Roland Garros" the same, since both expressions represent the same tournament. The x-axis of the line plot represents time and the y-axis represents winner-age. Our plot is faceted by tournament, as each line represents the winner age of each grand slam. 
```{r lineplot}
